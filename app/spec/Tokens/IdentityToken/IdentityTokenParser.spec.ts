/// <reference path='../../../../typings/main.d.ts' />

import {PjmtIdentityTokenParsed} from '../../../src/Tokens/IdentityToken/PjmtIdentityTokenParsed';
import {TokenDecoder} from '../../../src/Tokens/TokenDecoder';
import {TokenParser} from '../../../src/Tokens/TokenParser';

describe('IdentityTokenParser', () => {
    let token:string = "eyJ0eXAiOiJKV1QiLCJhbGciOiJSUzI1NiIsImtpZCI6ImEzck1VZ01Gdjl0UGNsTGE2eUYzekFrZnF1RSIsIng1dCI6ImEzck1VZ01Gdjl0UGNsTGE2eUYzekFrZnF1RSJ9..hS9vFGcUq6j3CBRsUDm2RG0zwyNbdRijN1L_f7EmX4d5o8Wxh9RaWxPmYUSAGt211lrnp1W0zm7HfWqOu3szJnhCWKfX_P9CYuc-3NyWXxL9iMZM07lzweunJeYrWM99KCWT9il_ptR3cvNBEZPBBHBmzQ5EPbHqMga35U3U0rHTuC1134qvDFE-2YoGyNXxP5KD-pyVlx0hPXOsfv9xYfKr9C31eEMqyyavZWpmiH8Ywz1vlJTMdX6Y9Gm7599Ojqe66mRAaDX7_rf8kecgvkSZD0uozHPGIX-Y8ORPS83AHXTOFOAXoO3hvxnJJ6iN1oVJSzWVsI59VFbvMIawCw";

  // beforeEach(() => {
  // });

  it('should parse a token', () => {
    
    let tokenDecoder = new TokenDecoder(token);
    let tokenDecoded = tokenDecoder.Decode();
    
    let tokenParser = new TokenParser(tokenDecoded);
    let tokenParsed = tokenParser.Parse(PjmtIdentityTokenParsed);
    
    let sid = tokenParsed.conteudoObject.sid;
    
    expect(sid).toBe("3d0d35f14acad73123a6caf0e95694b3");
    
    });
});
