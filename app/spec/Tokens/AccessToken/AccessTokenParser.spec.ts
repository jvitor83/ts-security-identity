...
import {PjmtAccessTokenParsed} from '../../../src/Tokens/AccessToken/PjmtAccessTokenParsed';
import {TokenDecoder} from '../../../src/Tokens/TokenDecoder';
import {TokenParser} from '../../../src/Tokens/TokenParser';
...
describe('AccessTokenParser', () => {
  ...
  it('should parse a token', () => {
    ...
    let tokenDecoder = new TokenDecoder(token);
    let tokenDecoded = tokenDecoder.Decode();
    
    let tokenParser = new TokenParser(tokenDecoded);
    let tokenParsed = tokenParser.Parse(PjmtAccessTokenParsed);
    
    //let idp = tokenDecoded.cabecalhoObject.typ;
    let client_id = tokenParsed.conteudoObject.client_id;
